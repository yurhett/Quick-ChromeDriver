...
class q_chromedriver:
    ...
    def _download_target(self, target_ver):
        ...
        if target_ver <= 73:
            ...
            try:
                urllib.request.urlretrieve('http://chromedriver.storage.googleapis.com/' + below73_map[
            ...
            try:
                url_read = urllib.request.urlopen(
                    urllib.request.Request('http://chromedriver.storage.googleapis.com/ ')).read().decode()
            except:
                print('Can\'t connect to the server! ')
                raise ConnectionError('Can\'t connect to the server')
            else:
                for i in re.findall('<Key>(.*?)</Key>', url_read):
                    if i[0].split('/')[
                        0] in 'qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM' or 'RELEASE' in i or int(
                            i.split('.')[0]) <= 72:
                        continue
                ...
                try:
                    print('Downloading... \nURL:http://chromedriver.storage.googleapis.com/' + available_versions[
                        str(target_ver)] + '/chromedriver_win32.zip')
                    urllib.request.urlretrieve('http://chromedriver.storage.googleapis.com/' + available_versions[
                        str(target_ver)] + '/chromedriver_win32.zip', 'chromedriver_win32.zip')
                except:
    ...
    def has_driver(self):
        if os.path.isfile('./chromedriver.exe'):
            ...
            if 'ChromeDriver' in res:
                ...
                print('This file is not a ChromeDriver')
                return None
        else:
            print('There is no Chrome Driver')
            return None
        ...
        if self.has_chrome() is None:
            ...
            if self.has_driver() is None:
                ...
                if self.chrome_ver != int(self.driver_ver[0]):
                    print(f"Wrong Version! Browser:{self.chrome_ver}, Driver:{self.driver_ver[0]}, Updating")
                    self._del_olddriver()
                    self._download_target(self.chrome_ver)
